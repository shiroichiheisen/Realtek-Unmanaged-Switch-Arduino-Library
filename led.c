...
static rtk_api_ret_t _rtk_led_groupAbility_set(rtk_led_group_t group, rtk_led_ability_t *pAbility)
{
    ...
    if (pAbility->link_10m == RTK_ENABLED)
        regData |= 0x0001;
    else
        regData &= ~0x0001;

    if (pAbility->link_100m == RTK_ENABLED)
        regData |= 0x0002;
    else
        regData &= ~0x0002;

    if (pAbility->link_500m == RTK_ENABLED)
        regData |= 0x0004;
    else
        regData &= ~0x0004;

    if (pAbility->link_1000m == RTK_ENABLED)
        regData |= 0x0008;
    else
        regData &= ~0x0008;

    if (pAbility->act_rx == RTK_ENABLED)
        regData |= 0x0010;
    else
        regData &= ~0x0010;

    if (pAbility->act_tx == RTK_ENABLED)
        regData |= 0x0020;
    else
        regData &= ~0x0020;
...
static rtk_api_ret_t _rtk_led_groupAbility_get(rtk_led_group_t group, rtk_led_ability_t *pAbility)
{
    ...
    pAbility->link_10m = (regData & 0x0001) ? RTK_ENABLED : RTK_DISABLED;
    pAbility->link_100m = (regData & 0x0002) ? RTK_ENABLED : RTK_DISABLED;
    pAbility->link_500m = (regData & 0x0004) ? RTK_ENABLED : RTK_DISABLED;
    pAbility->link_1000m = (regData & 0x0008) ? RTK_ENABLED : RTK_DISABLED;
    pAbility->act_rx = (regData & 0x0010) ? RTK_ENABLED : RTK_DISABLED;
    pAbility->act_tx = (regData & 0x0020) ? RTK_ENABLED : RTK_DISABLED;
...
{
    return _rtk_led_enable_set(group, pPortmask);
}
...
{
    return _rtk_led_enable_get(group, pPortmask);
}
...
{
    return _rtk_led_operation_set(mode);
}
...
{
    return _rtk_led_operation_get(pMode);
}
...
{
    return _rtk_led_modeForce_set(port, group, mode);
}
...
{
    return _rtk_led_modeForce_get(port, group, pMode);
}
...
{
    return _rtk_led_blinkRate_set(blinkRate);
}
...
{
    return _rtk_led_blinkRate_get(pBlinkRate);
}
...
{
    return _rtk_led_groupConfig_set(group, config);
}
...
{
    return _rtk_led_groupConfig_get(group, pConfig);
}
...
{
    return _rtk_led_groupAbility_set(group, pAbility);
}
...
{
    return _rtk_led_groupAbility_get(group, pAbility);
}
...
{
    return _rtk_led_serialMode_set(active);
}
...
{
    return _rtk_led_serialMode_get(pActive);
}
...
{
    return _rtk_led_OutputEnable_set(state);
}
...
{
    return _rtk_led_OutputEnable_get(pState);
}
...
{
    return _rtk_led_serialModePortmask_set(output, pPortmask);
}
...
{
    return _rtk_led_serialModePortmask_get(pOutput, pPortmask);
}
