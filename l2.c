...
static rtk_api_ret_t _rtk_l2_init(void)
{
    ...
    if ((retVal = rtl8367c_setAsicLutIpMulticastLookup(RTK_DISABLED)) != RT_ERR_OK)
        return retVal;

    /*Enable CAM Usage*/
    if ((retVal = rtl8367c_setAsicLutCamTbUsage(RTK_ENABLED)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_l2_ucastAddr_flush(rtk_l2_flushCfg_t *pConfig)
{
    ...
    if (pConfig->flushByVid == RTK_ENABLED)
    {
        ...
        if ((retVal = rtl8367c_setAsicLutFlushType((pConfig->flushStaticAddr == RTK_ENABLED) ? FLUSHTYPE_BOTH : FLUSHTYPE_DYNAMIC)) != RT_ERR_OK)
            return retVal;

        if (pConfig->flushAddrOnAllPorts == RTK_ENABLED)
        {
            if ((retVal = rtl8367c_setAsicLutForceFlush(RTL8367C_PORTMASK)) != RT_ERR_OK)
                return retVal;
        }
        else if (pConfig->flushByPort == RTK_ENABLED)
        {
    ...
    else if (pConfig->flushByFid == RTK_ENABLED)
    {
    ...
    else if (pConfig->flushByMac == RTK_ENABLED)
    {
...
static rtk_api_ret_t _rtk_l2_ipMcastAddrLookup_set(rtk_l2_ipmc_lookup_type_t type)
{
    ...
    {
        if ((retVal = rtl8367c_setAsicLutIpMulticastLookup(RTK_ENABLED)) != RT_ERR_OK)
            return retVal;

        if ((retVal = rtl8367c_setAsicLutIpMulticastVidLookup(RTK_DISABLED)) != RT_ERR_OK)
            return retVal;
    ...
    else if (type == LOOKUP_IP_VID)
    {
        ...
        if ((retVal = rtl8367c_setAsicLutIpMulticastVidLookup(RTK_ENABLED)) != RT_ERR_OK)
            return retVal;
...
static rtk_api_ret_t _rtk_l2_ipMcastAddrLookup_get(rtk_l2_ipmc_lookup_type_t *pType)
{
    ...
    if (enabled == RTK_ENABLED)
    {
        if (vid_lookup == RTK_ENABLED)
            *pType = LOOKUP_IP_VID;
        else
            *pType = LOOKUP_IP;
    }
...
static rtk_api_ret_t _rtk_l2_ipMcastGroupEntry_add(ipaddr_t ip_addr, rtk_uint32 vid, rtk_portmask_t *pPortmask)
{
    ...
    for (index = 0; index <= RTL8367C_LUT_IPMCGRP_TABLE_MAX; index++)
    {
        ...
        if ((valid == RTK_ENABLED) && (group_addr == ip_addr) && (group_vid == vid))
        {
        ...
        if ((valid == RTK_DISABLED) && (empty_idx == 0xFFFF)) /* Unused */
    ...
    pmask = physicalPortmask;
    if ((retVal = rtl8367c_setAsicLutIPMCGroup(empty_idx, ip_addr, vid, pmask, RTK_ENABLED)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_l2_ipMcastGroupEntry_del(ipaddr_t ip_addr, rtk_uint32 vid)
{
    ...
    for (index = 0; index <= RTL8367C_LUT_IPMCGRP_TABLE_MAX; index++)
    {
        ...
        if ((valid == RTK_ENABLED) && (group_addr == ip_addr) && (group_vid == vid))
        {
            ...
            group_vid = 0;
            pmask = 0;
            if ((retVal = rtl8367c_setAsicLutIPMCGroup(index, group_addr, group_vid, pmask, RTK_DISABLED)) != RT_ERR_OK)
                return retVal;
...
{
    return _rtk_l2_addr_add(pMac, pL2_data);
}
...
{
    return _rtk_l2_addr_get(pMac, pL2_data);
}
...
{
    return _rtk_l2_addr_next_get(read_method, port, pAddress, pL2_data);
}
...
{
    return _rtk_l2_addr_del(pMac, pL2_data);
}
...
{
    return _rtk_l2_mcastAddr_add(pMcastAddr);
}
...
{
    return _rtk_l2_mcastAddr_get(pMcastAddr);
}
...
{
    return _rtk_l2_mcastAddr_next_get(pAddress, pMcastAddr);
}
...
{
    return _rtk_l2_mcastAddr_del(pMcastAddr);
}
...
{
    return _rtk_l2_ipMcastAddr_add(pIpMcastAddr);
}
...
{
    return _rtk_l2_ipMcastAddr_get(pIpMcastAddr);
}
...
{
    return _rtk_l2_ipMcastAddr_next_get(pAddress, pIpMcastAddr);
}
...
{
    return _rtk_l2_ipMcastAddr_del(pIpMcastAddr);
}
...
{
    return _rtk_l2_ipVidMcastAddr_add(pIpVidMcastAddr);
}
...
{
    return _rtk_l2_ipVidMcastAddr_get(pIpVidMcastAddr);
}
...
{
    return _rtk_l2_ipVidMcastAddr_next_get(pAddress, pIpVidMcastAddr);
}
...
{
    return _rtk_l2_ipVidMcastAddr_del(pIpVidMcastAddr);
}
...
{
    return _rtk_l2_ucastAddr_flush(pConfig);
}
...
{
    return _rtk_l2_table_clearStatus_get(pStatus);
}
...
{
    return _rtk_l2_flushLinkDownPortAddrEnable_set(port, enable);
}
...
{
    return _rtk_l2_flushLinkDownPortAddrEnable_get(port, pEnable);
}
...
{
    return _rtk_l2_agingEnable_set(port, enable);
}
...
{
    return _rtk_l2_agingEnable_get(port, pEnable);
}
...
{
    return _rtk_l2_limitLearningCnt_set(port, mac_cnt);
}
...
{
    return _rtk_l2_limitLearningCnt_get(port, pMac_cnt);
}
...
{
    return _rtk_l2_limitSystemLearningCnt_set(mac_cnt);
}
...
{
    return _rtk_l2_limitSystemLearningCnt_get(pMac_cnt);
}
...
{
    return _rtk_l2_limitLearningCntAction_set(port, action);
}
...
{
    return _rtk_l2_limitLearningCntAction_get(port, pAction);
}
...
{
    return _rtk_l2_limitSystemLearningCntAction_set(action);
}
...
{
    return _rtk_l2_limitSystemLearningCntAction_get(pAction);
}
...
{
    return _rtk_l2_limitSystemLearningCntPortMask_set(pPortmask);
}
...
{
    return _rtk_l2_limitSystemLearningCntPortMask_get(pPortmask);
}
...
{
    return _rtk_l2_learningCnt_get(port, pMac_cnt);
}
...
{
    return _rtk_l2_floodPortMask_set(floood_type, pFlood_portmask);
}
/* Function Name:
...
{
    return _rtk_l2_floodPortMask_get(floood_type, pFlood_portmask);
}
...
{
    return _rtk_l2_localPktPermit_set(port, permit);
}
...
{
    return _rtk_l2_localPktPermit_get(port, pPermit);
}
...
{
    return _rtk_l2_aging_set(aging_time);
}
...
{
    return _rtk_l2_aging_get(pAging_time);
}
...
{
    return _rtk_l2_ipMcastAddrLookup_set(type);
}
...
{
    return _rtk_l2_ipMcastAddrLookup_get(pType);
}
...
{
    return _rtk_l2_ipMcastForwardRouterPort_set(enabled);
}
...
{
    return _rtk_l2_ipMcastForwardRouterPort_get(pEnabled);
}
...
{
    return _rtk_l2_ipMcastGroupEntry_add(ip_addr, vid, pPortmask);
}
...
{
    return _rtk_l2_ipMcastGroupEntry_del(ip_addr, vid);
}
...
{
    return _rtk_l2_ipMcastGroupEntry_get(ip_addr, vid, pPortmask);
}
...
{
    return _rtk_l2_entry_get(pL2_entry);
}
